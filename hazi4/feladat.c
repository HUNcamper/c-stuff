...
void titkosit(char bemenet[], char kimenet[]) {
    ...
    int szo_len_vege = i_ki;
    int i_be = 0;

    for (int i = 0; i < szo_len_vege; i++)
    {
        szo_len = kimenet[i] - '0';

        for (int j = i_be + szo_len - 1; j > i_be-1; j--)
        {
            kimenet[i_ki++] = bemenet[j];
        }
...
void call_1()
{
    char *p, bemenet[256]={0}, kimenet[256]={0};
    for (int i = 0; i < 255; bemenet[i] = kimenet[i] = '#', ++i);
