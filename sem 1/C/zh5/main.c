...
rombusz compare(rombusz r1, rombusz r2) {
    float r1_t = (r1.e * r1.f) / 2;
    float r2_t = (r2.e * r2.f) / 2;

    if(r1_t > r2_t || r1_t == r2_t) return r1;
    else return r2;
}
...
int szintemelkedes(int magassagok[], int pontok) {
    int szint = 0;
    if(magassagok[0] > 0) szint = magassagok[0];

    for (int i = 0; i < pontok-1; i++) {
        int kulonbseg = magassagok[i+1] - magassagok[i];

        if(kulonbseg > 0) szint += kulonbseg;
    }
